...
#                 raise HTTPException(
#                     400, f"التاريخ {check_date} محجوز بالفعل{clan_name}")
#             elif solo.status == ReservationStatus.pending_validation:
#                 raise HTTPException(400,
#                                     f"التاريخ {check_date} محجوز لكن غير مصدق عليه{clan_name}. "
#                                     f"تحقق مرة أخرى خلال {settings.validation_deadline_days} أيام")
...
#                                     f"أو احجز يوماً واحداً فقط في {date1}{clan_name}")
#             else:
#                 # Solo reservation where day 2 has mass wedding - not allowed
#                 raise HTTPException(400,
#                                     f"التاريخ {date2} محجوز لعرس جماعي{clan_name}. "
#                                     f"اختر تاريخاً آخر للحجز أو انضم للعرس الجماعي")
...
#                 raise HTTPException(
#                     400, f"التاريخ {check_date} محجوز بالكامل{clan_name}")

#             # Check if requesting solo when mass wedding exists
#             if not resv_in.join_to_mass_wedding and not resv_in.allow_others:
#                 raise HTTPException(400,
#                                     f"التاريخ {check_date} محجوز لعرس جماعي{clan_name}. "
#                                     f"انضم إذا كنت ترغب: {current_count}/{settings.max_grooms_per_date} عريس")
...
#         if not hall:
#             clan_name = f" في عشيرة {target_clan.name}" if not is_same_clan else ""
...
#             logger.error(
#                 f"PDF generation failed for reservation {resv.id}: {pdf_error}")
...
#             "pdf_url": f"/download/{resv.id}" if resv.pdf_url else None
#         }
...
#         raise HTTPException(500, f"خطأ في النظام: {str(e)}")
#     finally:
...
#     ).all()
#     if not resv:
#         raise HTTPException(status_code=404, detail="لا توجد حجوزات")
#     return resv
...
#     ).first()
#     if not resv:
#         raise HTTPException(status_code=404, detail="لا يوجد حجز معلق")
#     return resv
...
#     ).first()
#     if not resv:
#         raise HTTPException(status_code=404, detail="لا يوجد حجز مصدق عليه")
#     return resv
...
#     ).all()
#     if not resv:
#         raise HTTPException(status_code=404, detail="لا يوجد حجز ملغي")
#     return resv
